...
| 4 | 無料プランの比較 `#compare` | Cairn / Slack / 国産チャット / Backlog の比較表。日付と出典の注記つき |
...
- Slack / Backlog は比較表・FAQ・meta に出してよい。Hero の見出しには出さない
...
- OGP 画像（`og-image.png` / `.svg`）は旧コピー「One Project. One Place.」のまま。新しい見出しで作り直す
