...
1. `enums.ts` / `workspaces.ts` にスキーマ追加 → `pnpm db:generate` → マイグレーション適用。
2. `permissions.ts` の `getWorkspaceRole` を active 限定に変更（テスト追加）。
3. `get-auth-context.ts` のワークスペース解決から非活性を除外。
4. 非活性化 / 再活性化 API（admin 限定・最後の owner 保護）。
5. メンバー一覧 API に `status`、候補リスト系を active 絞り込み。
6. UI（現役 / 卒業生分離・操作ボタン・確認ダイアログ）。
7. 動作確認（非活性ユーザーの 403・再活性化・履歴保持）。
