...
Cairn は部活動でも利用される想定で、卒業生など「いったん抜けるが、いずれコーチとして戻る可能性がある」ユーザーが存在する。設計開始時点では、ユーザーを外す導線（ワークスペースからの除名・退会・アカウント削除）は **API・UI ともに未実装**だった。
...
→ 設計方針との整合: **通常運用＝非活性化**、**本人によるアカウント削除＝投稿コンテンツの削除 + 共同作業構造の匿名化パス**とする。`profiles` は共同作業の参照整合性のため匿名化して保持し、Supabase Authのユーザーは物理削除する。
...
## 7. フェーズ2: 本人操作のアカウント削除（Issue #469）

App Storeのアカウント削除要件および本人の削除請求へ対応する。非活性化（§3 の `status`）とは独立した、全ワークスペース横断の不可逆操作とする。

- `DELETE /api/me/account` は認証済み本人だけを対象とし、確認文字「削除」を要求する。
- 最後のactive ownerであるワークスペースがあれば、owner移譲が済むまで削除を拒否する。
- 本人のメッセージ本文、写真、添付ファイル、ギャラリーコメント、個人のAI会話および個人用設定・連携・通知を削除する。
- Storage削除対象はDB outboxへ保存し、非同期削除の送信失敗時もcronから再送する。
- `profiles` 行は**残したまま**、表示名を「退会済みユーザー」へ置換し、bio等のPIIを消去する。プロジェクト・タスク等の共同作業構造は匿名名義で保持する。
- 全workspace membershipを削除した後、Stripe顧客とSupabase Authユーザーを削除する。途中で失敗しても、残っているAuthセッションから同じ操作を再試行できる順序にする。
- Expo版の設定は認証済みWebViewであるため、同じ画面から削除し、成功時のbridge messageでネイティブ側セッションも破棄する。
...
5. **匿名化フェーズの優先度** — 解決済み。App Store提出前にIssue #469として実装する。
...
| 6 | **（フェーズ2）本人操作のアカウント削除** | §7。本人投稿の削除、表示名置換、PII消去、Storage outbox、Auth削除、Expoログアウトを検証 | #1（独立着手可） | 実装済み（[#469](https://github.com/keishingu/Cairn/issues/469)、main反映・TestFlight確認待ち） |
