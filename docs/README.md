...
| FDEによる顧客別ダッシュボード設計 | `fde-dashboard-customization-design.md`（未実装） | `ai-era-pm-strategy.md`（監視ダッシュボードとの境界）、`09_product_strategy_notes.md`（業種別テンプレート戦略） |
...
| [`lp-content-redesign.md`](./lp-content-redesign.md) | LP コンテンツ（無料のチャット＋プロジェクト管理、比較表・料金、コピー規則、CTA/UTM 規約） | 2026-09-23 |
...
| [`fde-dashboard-customization-design.md`](./fde-dashboard-customization-design.md) | FDEが顧客固有の判断基準を宣言的なレシピへ落とし、Jevで表示を選別する個人向けダッシュボード設計（未実装） | 2026-09-23 |
