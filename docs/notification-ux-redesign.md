...
| ファイル添付 | unread++ のみ | なし（**廃止済み**。知らせたい場合はメンションを付ける運用） | なし |
| プロジェクトのステータス変更 | — | なし（通知しない。需要が出たら Phase 4 で管理者向けオプトイン） | なし |
| タスク割り当て | — | あり | あり |
...
| 論点 | 推奨 | 理由 |
|---|---|---|
| ファイル添付通知の扱い | 廃止（バッジのみ）— ✅ 採用・実装済み | Slack/Discord/Notion いずれも添付だけでは通知しない。ノイズ源 |
| プロジェクトのステータス変更通知 | 実装しない — ✅ 採用 | 関心を持つのは管理者以上に限られる。需要が出たら Phase 4 の通知設定でオプトイン提供 |
