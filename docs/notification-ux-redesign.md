...
2. **既読は全デバイスで即時同期し、閲覧中メンションはバッジを付けずに通知する**
3. **通知は必ずソースへのディープリンク**
...
| **Push** | 「今すぐ知るべき」ものだけ。閲覧状態に応じてバッジを制御 | Inngest → web-push / Expo | イベント駆動 |
...
| @メンション | unread++ & mention++（閲覧中は即時既読） | あり | あり（**閲覧中も送信、アプリアイコンバッジ更新なし**） |
