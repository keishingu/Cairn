...
- メンション通知作成とチャンネル既読化は、同じ `channel_read_states` 行をロックするトランザクションで直列化する。既読位置は既存値と取得した最新メッセージの新しい方を採用して単調増加させ、その時刻以前と削除済みメッセージ由来の通知だけを既読化した後、残った未読メンション通知行から `unread_mention_count` を再計算する。空チャンネルでは既読位置を未来へ進めず、未参加者の初期 read state は対象メッセージ直前を起点にして、その後のメンションを未読として残す
