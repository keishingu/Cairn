...
- メンション通知作成とチャンネル既読化は、同じ `channel_read_states` 行をロックするトランザクションで直列化する。既読位置は既存値と取得した最新メッセージの新しい方を採用して単調増加させ、その時刻以前の通知だけを既読化した後、残った未読メンション通知行から `unread_mention_count` を再計算する。未参加者の初期 read state は対象メッセージ直前を起点にし、そのメンションを未読として残す
