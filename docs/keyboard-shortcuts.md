...
| Calendar | `⌥N`新規プロジェクト、`⌥M/W/A`月/週/タイムライン、`⌥T`今日へ、`⌥←→`期間送り、`⌥F`フィルター |
...
| Settings | プロフィール: `⌘⌥+0`、`Esc`ダイアログ閉じる |
...
3. **第2.5段（◎・実装済み）**: 数字ナビ拡張（5〜0 + `,`）/ コンテキストショートカット拡張（`⌥F`フィルタ / `⌥S`検索 / `⌥G/T`ビュー切替 / `⌥A`タイムライン / `⌥T`今日 / `⌥D`詳細パネル / `⌥[/]`フィルタタブ / `⌥⏎`完了トグル / `⌥⌫`削除 / `⌥R`再インデックス）。
