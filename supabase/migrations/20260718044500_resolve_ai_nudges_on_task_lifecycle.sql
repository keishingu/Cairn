...
-- projectのCASCADE削除では、子taskのDELETE時点で親FKが参照できないため、親の削除前に解消する。
CREATE OR REPLACE FUNCTION public.resolve_ai_nudges_before_project_delete()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
END;
$$;--> statement-breakpoint
REVOKE ALL ON FUNCTION public.resolve_ai_nudges_before_project_delete() FROM PUBLIC;--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION public.resolve_ai_nudges_before_project_delete();
