...
- **外部サービスは機能カテゴリごとに排他接続する**: ワークスペースごとに calendar / chat / storage / meeting の主系 provider を1つだけ選ぶ。Google Calendar を選んだらカレンダー、Slack を選んだらチャット、Box を選んだらストレージ、Circleback を選んだら会議記録の正本として使い、Cairn 内部機能との並行運用や失敗時の暗黙フォールバックはしない。手動で行う対応付けは project / milestone ↔ Google Calendar event / Slack channel / Box folder、および project ↔ Circleback tag。Circleback webhook は tag mapping から provider-neutral な `meeting` へ取り込むが、議事録・transcript・Circleback action items はCairnで編集せず、`Circlebackで編集` から正本へ誘導する。詳細は [`docs/exclusive-service-integration-design.md`](docs/exclusive-service-integration-design.md)
