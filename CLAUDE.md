...
- **Mobile (Expo) は `apps/mobile/`**: チャット以外は WebView で Web 版を表示する方針。ネイティブ化のロードマップは [`docs/08_expo_roadmap.md`](docs/08_expo_roadmap.md) を参照
  ...
  - **Google ログインはネイティブ実装**: Web のリダイレクト方式は使えないため、`expo-web-browser` で認可コードを受け取り Supabase の PKCE フロー（`exchangeCodeForSession`）で交換する（`apps/mobile/lib/oauth.ts`）。redirect 先はアプリスキーム `cairn://auth/callback`。**Supabase の許可リストに登録が必要**（ローカルは `supabase/config.toml` の `additional_redirect_urls`、本番は Supabase ダッシュボードの Redirect URLs）。初回ログイン時も `/api/auth/setup` を呼んで profiles を作成する。Google プロバイダはローカルでは既定で無効。`supabase/config.toml` の `[auth.external.google]` を `enabled = true` にし、`AUTH_GOOGLE_CLIENT_ID` / `AUTH_GOOGLE_SECRET` を env で渡して `supabase` 再起動が必要（Web は本番 Supabase 側で有効化済み）
  - **ネイティブモジュール/設定を変更したら runtimeVersion を上げる**: OTA(`eas update`) は JS しか差し替えられず、ネイティブモジュール追加は配信できない。`app.json` の `runtimeVersion` を固定文字列で運用しているため、ネイティブ構成（依存・`app.json`・`eas.json`）を変えたら手動で上げること。上げないと不整合な旧バイナリに OTA が降って `Cannot find native module ...` で silent crash する。ネイティブ変更を含む PR は `.github/workflows/mobile-build.yml` が EAS Build を走らせ、インストール可能なバイナリを作る（OTA プレビューの `mobile-preview.yml` とは別物）
