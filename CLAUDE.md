...
- **Electron は `apps/desktop/`**。リポジトリルートから `pnpm desktop:dev` / `pnpm desktop:build:prod` / `pnpm desktop:build:dev` を使う。アイコン生成は `pnpm --filter @cairn/desktop generate-icons`。接続先と生成パスは [`README.md`](README.md#electron-デスクトップアプリ) を参照
...
- **設定の保存先は値の形で決める**: 人がワークスペースをまたいで持ち、取りうる値が閉じた真偽値か列挙で、全行に既定が要り、不正値を DB で拒否し、Web とアプリが `GET /api/me` のフィールドとして読むものは `profiles` のカラムにする。`NOT NULL` と `CHECK` を付ける。該当は `theme`、`accent_id`、`ai_nudges_enabled`、`calendar_week_start`。無くてもよい、`null` で未設定を表す、リストや入れ子で項目追加のたびにマイグレーションしたくない、1件を読んで一部だけマージして書き戻すものは JSON にする。今の JSON は `workspaces.settings`（`projectLabel`、`coverPhotos`）だけである。ワークスペースの AI 巡回のオンオフは真偽値で既定があるため、同じテーブルに JSON があっても専用カラムのままにする。同じ形の個人設定が増えてカラムと `CHECK` を毎回足す負担が大きくなった時点で、`profiles` の表示設定用 JSON を検討する
