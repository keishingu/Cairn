...
- **Mobile (Expo) は `apps/mobile/`**: チャット以外は WebView で Web 版を表示する方針。ネイティブ化のロードマップは [`docs/08_expo_roadmap.md`](docs/08_expo_roadmap.md) を参照
  ...
  - PR の Mobile Preview は、対象 commit の Vercel Preview URL と共有 Supabase Preview 設定を EAS の `preview` 環境へ同期し、`eas update --environment preview` で Development Build 向けに配信する（`.github/workflows/mobile-preview.yml`）。同一 PR の古い実行はキャンセルし、異なる PR は EAS 同期直前の FIFO ゲートで直列化する
...
- **AIエージェントは、ユーザーから `push`・PR 作成・PR 更新を明示的に依頼されるまでリモートへ push しない**。ローカルでのブランチ作成・コミットは可とする
- push が許可された作業でも、途中経過やレビュー指摘1件ごとには push しない。関連する変更・テスト・セルフレビューをローカルでまとめ、原則として作業の最後に1回だけ push する
- ユーザーが `$pr-green-loop` を明示的に依頼した場合は例外とし、その PR・その goal の終了条件を満たすまで、CI・AI レビュー対応に必要な複数回の push を許可する。同時点で判明している修正はローカルでまとめ、指摘1件ごとではなく1ループにつき1回の push とする
- Vercel Preview やリモート CI でしか確認できない事項があり、追加の push が必要な場合は、目的と確認内容をユーザーへ説明してから明示的な許可を得る
- `$pr-green-loop` の例外を除き、一度の push 許可を、その後に発生した別作業や追加レビュー対応への継続的な許可として扱わない。例外の許可も goal の完了・中断・別 PR への移動時に失効する
