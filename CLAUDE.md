...
- **Mobile (Expo) は `apps/mobile/`**: チャット以外は WebView で Web 版を表示する方針。ネイティブ化のロードマップは [`docs/08_expo_roadmap.md`](docs/08_expo_roadmap.md) を参照
  ...
  - **Apple の追加ログイン手段は設定から手動連携**: 同一メールの自動紐付けに加え、設定 → アカウント → ログイン方法で `linkIdentity` により既存アカウントへ Apple を明示連携できる（relay email 対応）。ローカルは `enable_manual_linking = true`、Preview / Production は Dashboard でも Manual Linking を有効化する。Expo iOS の設定 WebView はネイティブ bridge（`link-apple-identity`）経由で ID token 連携する
