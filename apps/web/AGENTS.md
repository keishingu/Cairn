...
- **表示文言の i18n キーは英語の UI 文**（`t('Sign in with Google')`）。日本語は `packages/shared` のカタログ。言語は `profiles.locale`（`ja` / `en` / `system`）で、`system` は Accept-Language と `navigator.languages` を同じ順で解決する。URL にロケールを付けない。ワークスペースのプロジェクト呼び方は未設定（null）のときだけ訳し、保存済みの文字列はそのまま出す
