...
- 権限チェックは `src/lib/permissions.ts`（`requireWorkspaceOwner` / `Admin` / `Member`）。403 はロールを明示した日本語メッセージにし、フロントに生の 401/403 を出さない。UI は `useWorkspacePermissions()` で事前に disable・非表示にする
