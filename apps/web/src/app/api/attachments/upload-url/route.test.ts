...
describe('/api/attachments/upload-url', () => {
  ...
  it('拡張子が.csvでapplication/octet-streamでもtext/csvに正規化する', async () => {
    const { POST } = await import('./route')
    const res = await POST(post({ channelId: CHANNEL_ID, fileName: 'data.csv', mimeType: 'application/octet-stream', fileSize: 100 }))
