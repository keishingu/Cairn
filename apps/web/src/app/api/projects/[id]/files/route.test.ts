...
describe('GET /api/projects/[id]/files', () => {
  ...
  it('pending project upload を一覧から除外する', async () => {
    ...
    mockDb.select
      ...
      .mockReturnValueOnce(chain(rows, { resolveOn: 'offset' }))
  ...
  it('候補取得を 200 件ずつに分けて全ページを走査する', async () => {
    getAuthContext.mockResolvedValue({
      ctx: { workspaceId: 'ws-1', userId: 'user-1' },
      error: null,
    })
    ...
    const secondPage = Array.from({ length: 3 }, (_, index) => ({
      id: `tail-${index}`,
      fileName: `tail-${index}.txt`,
      mimeType: 'text/plain',
      fileSize: 20,
      fileType: 'file',
      uploaderName: 'User Two',
      createdAt: new Date(`2026-07-03T00:0${index}:00Z`),
      metadata: {},
      workspaceId: 'ws-1',
      projectId: 'project-1',
      uploadedBy: 'user-3',
    ...
    mockDb.select
      .mockReturnValueOnce(chain([{ id: 'project-1' }]))
      .mockReturnValueOnce(chain(firstPage, { resolveOn: 'offset' }))
      .mockReturnValueOnce(chain(secondPage, { resolveOn: 'offset' }))
    mockDb.selectDistinct.mockReturnValueOnce({
      from: vi.fn().mockReturnThis(),
      where: vi.fn().mockResolvedValue([]),
    })
    ...
    expect(res.status).toBe(200)
    expect(json).toHaveLength(203)
    expect(mockDb.select).toHaveBeenCalledTimes(3)
    expect(mockCanAccessFile).toHaveBeenCalledTimes(203)
  })
})
