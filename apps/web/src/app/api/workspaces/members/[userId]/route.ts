...
export async function PATCH(
  ...
  try {
    ...
    const { workspaceMembers, activeWorkspaceMembers, profiles } = await import('@cairn/db')
    ...
    const response = await db.transaction(async (tx) => {
      ...
      if (target.kind === 'bot') {
        return NextResponse.json({ error: 'bot メンバーのロールは変更できません' }, { status: 422 })
      }
...
async function handleStatusChange(
  ...
  try {
    const { db } = await import('@cairn/db')
    const { workspaceMembers, profiles } = await import('@cairn/db')
    ...
    if (target.kind === 'bot') {
      return NextResponse.json({ error: 'bot メンバーの活性状態は変更できません' }, { status: 422 })
    }
