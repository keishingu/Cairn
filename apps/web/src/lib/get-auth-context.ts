...
export async function getAuthContext(): Promise<AuthResult> {
  ...
  try {
    ...
    const requestedWorkspaceId = cachedWorkspaceId ?? preferredWorkspaceId
    if (requestedWorkspaceId) {
      const preferred = memberships.find((membership) => membership.workspaceId === requestedWorkspaceId)
      if (preferred) {
