...
// デコードすると文字化けする。fatal:trueのTextDecoderは不正なUTF-8バイト列で例外を
// 投げる(=有効なUTF-8がたまたまU+FFFDを含む場合と、真にデコード不能な場合を区別できる)
// ため、それをもってCP932へフォールバックする。TextDecoderは既定で先頭のUTF-8 BOMも
// 自動的に取り除く。
