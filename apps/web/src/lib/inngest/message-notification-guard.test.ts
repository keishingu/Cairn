...
describe('メッセージ通知の送信元ガード', () => {
  ...
  it('送信者がactiveでメッセージが未削除ならロック中に処理する', async () => {
    ...
    await expect(runForActiveMessageSender('message-1', 'workspace-1', 'user-1', action)).resolves.toBe(
      'sent',
    )
  ...
  it('退会済みまたは削除済みなら通知処理を呼ばない', async () => {
    ...
    const { runForActiveMessageSender } = await import('./message-notification-guard')
    await expect(runForActiveMessageSender('message-1', 'workspace-1', 'user-1', action)).resolves.toBeNull()
