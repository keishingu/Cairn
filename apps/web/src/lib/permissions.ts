...
import { getWorkspaceRole, isWorkspaceMember, requireActiveMember } from './access/membership'

// role の解決・判定・active 要求は access/membership に一元化した（active_workspace_members
// ビュー経由）。既存の import 互換のため、ここから re-export する。
export {
  getWorkspaceRole,
  isWorkspaceOwner,
  isWorkspaceAdmin,
...
function hasWorkspaceLevelExternalUrl(metadata: unknown): boolean {
  const meta = (metadata ?? {}) as Record<string, unknown>
  return typeof meta['externalUrl'] === 'string'
}
...
export async function canAccessFile(
  ...
  if (
    file.uploadedBy === userId &&
    !file.projectId &&
    !pendingMetadataChannelId &&
    metadataChannelIds.size === 0 &&
    hasWorkspaceLevelExternalUrl(file.metadata)
  ) {
    return true
  }
