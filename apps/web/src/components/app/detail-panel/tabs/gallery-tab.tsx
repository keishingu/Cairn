...
export const GalleryTab = ({ projectId }: { projectId: string }) => {
  ...
  const upload = useMutation({
    ...
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['project-gallery', projectId] }),
  })
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 12, cursor: upload.isPending ? 'default' : 'pointer',
              fontFamily: 'inherit', opacity: upload.isPending ? 0.6 : 1,
            }}
          >
            <Icon name="upload" size={13}/>
            {upload.isPending ? 'アップロード中...' : '写真を追加'}
          </button>
        </div>

        {upload.isError && (
          <div style={{ marginBottom: 8, padding: '6px 10px', borderRadius: 6, background: 'var(--red-soft)', color: 'var(--red-text)', fontSize: 12 }}>
            {upload.error instanceof Error ? upload.error.message : 'アップロードに失敗しました'}
          </div>
        )}
