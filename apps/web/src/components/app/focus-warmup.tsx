...
//
// /api/warmup は認証済みユーザーのみ受け付けるため fetchWithAuth で Bearer トークンを付与する
// （偽装可能なリクエストヘッダだけでは外部からの連打を防げないため。詳細は route.ts）
...
export function FocusWarmup() {
  ...
  React.useEffect(() => {
    const warmup = () => {
      ...
      void fetchWithAuth('/api/warmup', { method: 'GET', cache: 'no-store', keepalive: true }).catch(() => {})
    }
