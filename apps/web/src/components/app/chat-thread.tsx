...
import { filterActiveWorkspaceMembers } from '@/lib/chat/active-workspace-members'
...
const GOOGLE_DOCS_URL_RE =
  /https:\/\/(?:docs\.google\.com\/(?:document|spreadsheets|presentation)\/d\/[a-zA-Z0-9_-]+(?:\/[^\s]*)*|drive\.google\.com\/file\/d\/[a-zA-Z0-9_-]+(?:\/[^\s]*)*)/g
...
function toPlainSnippet(content: string, nameOf?: (userId: string) => string | undefined): string {
  return stripMentionsToText(content, nameOf).replace(/\s+/g, ' ').trim()
}
...
export const ChatMessage = React.memo(function ChatMessage({
  ...
  React.useEffect(() => {
    const onEditEvt = (e: Event) => {
      if ((e as CustomEvent<string>).detail === messageId && isOwn) startEdit()
    }
    const onReactEvt = (e: Event) => {
      if ((e as CustomEvent<string>).detail === messageId) setShowPicker(true)
    }
    const onDeleteEvt = (e: Event) => {
      if ((e as CustomEvent<string>).detail === messageId && isOwn) setDeleteConfirm(true)
    }
  ...
  const messageActions = !editMode && (isMobile || hovered) && (
    ...
    >
      ...
      <button
        ...
        style={{ ...iconBtnStyle, color: bookmarked ? 'var(--accent)' : 'var(--text-3)' }}
      >
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          <span style={{ fontSize: compact ? 13 : 14, fontWeight: 700, color: 'var(--text)' }}>
            {senderName}
          </span>
        ...
        {replyTo && (
          <button
            ...
            style={{
              ...
              background: 'transparent',
              cursor: replyTo.isDeleted ? 'default' : 'pointer',
              width: '100%',
              maxWidth: '100%',
              minWidth: 0,
              overflow: 'hidden',
          ...
          >
            ...
            <span
              style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--text-3)', flexShrink: 0 }}
            >
              {replyTo.senderName}
            </span>
            <span
            ...
            >
              {replyTo.isDeleted
                ? '削除されたメッセージ'
                : toPlainSnippet(replyTo.content, (id) => mentionNames?.get(id)) ||
                  '（添付ファイル）'}
            </span>
          </button>
        )}
        {editMode ? (
          ...
          content && (
            <div
              style={{
                fontSize: emojiOnly ? 40 : compact ? 13 : 13.5,
                color: 'var(--text-2)',
                lineHeight: emojiOnly ? 1.2 : 1.6,
              }}
            >
              {emojiOnly ? (
                content
        ...
        <div
          style={{ marginTop: 6, display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center' }}
        >
          {reactions.map((r, i) => (
            <span
              key={i}
...
const ChatInputBar = ({
  ...
  const handleKeyDownWithMention = (e: React.KeyboardEvent, fallback: () => void) => {
    if (mentionCandidates.length > 0) {
      ...
      if (e.key === 'ArrowDown') {
        e.preventDefault()
        setSelectedIdx((i) => (i + 1) % mentionCandidates.length)
        return
      }
      if (e.key === 'ArrowUp') {
        e.preventDefault()
        setSelectedIdx((i) => (i - 1 + mentionCandidates.length) % mentionCandidates.length)
        return
      }
      if (e.key === 'Enter' && !e.shiftKey) {
        e.preventDefault()
      ...
      >
        {mentionCandidates.map((m, i) => (
          <button
            ...
            onMouseDown={(e) => {
              e.preventDefault()
              insertMention(m.userId, m.displayName)
            }}
          ...
          >
            ...
            <span
              style={{
                fontSize: 13.5,
                color: i === selectedIdx ? 'var(--accent)' : 'var(--text-2)',
                fontWeight: 500,
              }}
            >
              {m.displayName}
  ...
  const draftOverlay = React.useMemo(() => {
    ...
    while ((match = re.exec(draft)) !== null) {
      if (match.index > last)
        nodes.push(
          <span key={`t${last}`} style={{ color: 'var(--text)' }}>
            {draft.slice(last, match.index)}
          </span>,
        )
    ...
    if (last < draft.length)
      nodes.push(
        <span key="last" style={{ color: 'var(--text)' }}>
          {draft.slice(last)}
        </span>,
      )
    return nodes
  ...
  const canSend =
    (draft.trim().length > 0 || pendingAttachments.length > 0) && !isPending && !isUploading
    ...
    pendingAttachments.length > 0 || isUploading ? (
      <div
        style={{
          ...
          gap: 6,
          flexWrap: 'wrap',
          padding: compact ? '6px 10px 0' : '6px 14px 0',
        }}
      >
    ...
    return (
      ...
      >
        ...
        <div
          style={{
            background: 'var(--card-2)',
            border: `1px solid ${sendError ? 'var(--red)' : 'var(--border)'}`,
            borderRadius: 10,
            overflow: 'hidden',
          }}
        >
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, padding: '7px 10px' }}>
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <textarea
                ...
                style={{
                  ...
                  resize: 'none',
                  fontSize: 13,
                  color: draftOverlay ? 'transparent' : 'var(--text)',
                  caretColor: 'var(--text)',
                  outline: 'none',
                  fontFamily: 'inherit',
                  lineHeight: 1.5,
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: 'none',
                background: canSend ? 'var(--accent)' : 'var(--border-2)',
                color: canSend ? 'var(--on-accent)' : 'var(--text-4)',
  ...
  return (
    <div
      style={{ padding: '8px 24px 18px', background: 'var(--bg)', position: 'relative' }}
      {...dropHandlers}
    >
      {hiddenFileInput}
      {hiddenImageInput}
      ...
      <div
        style={{
          background: 'var(--card)',
          border: `1px solid ${sendError ? 'var(--red)' : 'var(--border-2)'}`,
          borderRadius: 12,
          boxShadow: 'var(--shadow-sm)',
          overflow: 'hidden',
        }}
      >
        ...
        {AttachmentPreviews}
        <div
          style={{ display: 'flex', alignItems: 'flex-end', gap: 10, padding: '10px 14px 12px' }}
        >
          <div style={{ flex: 1, position: 'relative' }}>
            {MentionPicker}
            {typeof placeholder !== 'string' && !draft && (
...
export const ChatThread = ({
  ...
  const persistDraft = React.useCallback(
    (id: string, text: string, attachments: PendingAttachment[]) => {
  ...
  const isAccessDenied =
    messagesError instanceof ChannelMessagesError && messagesError.status === 403
  const { data: wsMembers = [] } = useWorkspaceMembers()
  const activeWorkspaceMembers = React.useMemo(
    () => filterActiveWorkspaceMembers(wsMembers),
    [wsMembers],
  )
  ...
  const handleCheckboxToggle = React.useCallback(
    async (messageId: string, index: number, checked: boolean) => {
      try {
  ...
  const mentionMembers = React.useMemo(() => {
    ...
    if (chMemberIds.length > 0) {
      ...
      return activeWorkspaceMembers.filter(
        (m) => idSet.has(m.userId) && m.userId !== currentUser?.id,
      )
    }
    ...
    if (projectId) {
      ...
      return activeWorkspaceMembers.filter(
        (m) =>
          m.userId !== currentUser?.id && (m.role !== 'guest' || projectMemberIds.has(m.userId)),
      )
    }
    return activeWorkspaceMembers.filter((m) => m.userId !== currentUser?.id)
  }, [activeWorkspaceMembers, chMemberIds, currentUser?.id, projectId, projectMembers])
  ...
  return (
    ...
    >
      ...
      <div
        ref={scrollRef}
        style={{ flex: 1, overflow: 'auto', padding: compact ? '8px 0 16px' : '16px 0' }}
      >
        {isLoading ? (
          <div
            style={{
