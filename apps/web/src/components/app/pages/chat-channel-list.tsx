...
const ChatSidebarCollapsibleSection = ({
  title,
  count,
...
export const ChatSidebarItem = ({
  ...
  <button
    ...
    onMouseEnter={(e) => {
      if (!active && !mobile) (e.currentTarget as HTMLElement).style.background = 'var(--card)'
    }}
    onMouseLeave={(e) => {
      if (!active && !mobile) (e.currentTarget as HTMLElement).style.background = 'transparent'
    }}
  >
    {prefix === 'lock' ? (
      <span
        style={{
          ...
          alignItems: 'center',
          justifyContent: 'center',
          borderRadius: mobile ? 8 : undefined,
          background: mobile ? 'var(--card-2)' : undefined,
          color: 'var(--text-3)',
        }}
      >
      ...
      mobile ? (
        ...
        <span style={{ fontSize: 13, color: 'var(--text-3)', width: 14, textAlign: 'center' }}>
          {prefix}
        </span>
      )
    ) : null}
        ...
        {dot && (
          <span
            style={{
              ...
              borderRadius: '50%',
              background: dot,
              border: `2px solid var(--${mobile ? 'bg' : 'card-2'})`,
            }}
          />
        )}
      </div>
    ...
    {memberCount != null && !mobile && memberCount > 0 && (
      <span style={{ fontSize: 10.5, color: 'var(--text-4)', fontWeight: 500, flexShrink: 0 }}>
        {memberCount}名
      </span>
    )}
...
export const ChannelList = ({
  ...
  const activeMembers = React.useMemo(() => filterActiveWorkspaceMembers(members), [members])
  ...
  return (
    <div
      style={{
        flex: 1,
        overflow: 'auto',
        padding: isMobile ? '8px 0' : '8px 6px',
    ...
    >
      ...
      {archivedProjectChannels.length > 0 && (
        <ChatSidebarCollapsibleSection
          title="アーカイブ済み"
          count={archivedProjectChannels.length}
        >
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        >
          ...
          <DmPicker members={activeMembers} onStartDm={onStartDm} />
        </div>
        <div>
          {dms.map((d) => (
            <ChatSidebarItem
