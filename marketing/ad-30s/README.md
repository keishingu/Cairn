...
- 出力: 横長 1920×1080（YouTube など）と縦型 1080×1920（Instagram リール・ストーリーズ、YouTube ショート）。どちらも 60fps / H.264 + AAC ステレオ（BGM・効果音付き）
...
./marketing/ad-30s/build.sh                    # → out/cairn-ad-30s.mp4（横長）
FORMAT=vertical ./marketing/ad-30s/build.sh    # → out/cairn-ad-30s-vertical.mp4（縦型 9:16）
```
...
- 静止画で確認するだけなら `node marketing/ad-30s/render.mjs --preview 4.1 12.5 28` → `preview/`（縦型は先頭に `FORMAT=vertical`）
- ブラウザで開く場合は `index.html`（横長）/ `index.html?format=vertical`（縦型）。`render(t)` をコンソールから呼ぶと任意の時刻を表示できる
...
同じ `index.html` を `?format=vertical` で開くと `html.v` が付き、縦型用の CSS（`/* 縦型（9:16）レイアウト */` 以下）と JS の座標（`V ? ... : ...`）に切り替わる。テロップは `<br class="vbr">` で縦型のときだけ改行する。リールやショートは画面下部にキャプションや操作ボタンが重なるため、主要な要素は上から 1500px 程度までに収めている。

## 変更時の注意

- **料金・無料枠・比較の数値は LP と一致させる。** LP の文言を変えたら、この広告のシーン C / F / G / I も見直す
- レイアウトを変えたら横長・縦型の両方を `--preview` で確認する
